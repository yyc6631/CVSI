    ...
    def __getitem__(self, index) -> dict:
        try:
            if self.mode == 'relative':
                ...
                blip_target_img = self.blip_transform(PIL.Image.open(image_path).convert('RGB'))
                return {
                    'image': image,
                    'image_name': image_name,
                    'blip_img': blip_target_img
                }
...
class CIRCODataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            blip_target_img = self.blip_transform(PIL.Image.open(img_path).convert('RGB'))
            return {
                'image': img,
                'image_name': img_id,
                'blip_img': blip_target_img
